...
test("offline packed 0.3.2 artifact emits midnight timestamps before noon", async () => {
  ...
  try {
    ...
    assert.equal(packed.version, "0.3.2");
    await exec("tar", ["-xzf", join(temporary, packed.filename), "-C", temporary]);
    const artifact = join(temporary, "package");
    assert.equal(JSON.parse(await readFile(join(artifact, "package.json"), "utf8")).version, "0.3.2");
    assert.equal((await exec(process.execPath, [join(artifact, "src/cli.js"), "--version"])).stdout.trim(), "0.3.2");
