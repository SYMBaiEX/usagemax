...
function AccountPanel() {
  ...
  useEffect(() => {
    if (!linkCode || linkStatus?.redeemedAt) return;
    const timer = window.setInterval(() => setNow(Date.now()), 15_000);
    return () => window.clearInterval(timer);
  }, [linkCode, linkStatus?.redeemedAt]);
  ...
  const canManageOwnCollectors = account.capabilities["collectors:self"] === true || canManageCollectors;
  ...
  return <div className="account-layout"><aside className="account-sidebar"><span className="section-index">Your workspace</span><nav aria-label="Workspace sections"><a href="#overview">Overview</a>{account.profile ? <><a href="#account-profile">Profile & visibility</a><a href="#link-computer">Link a computer</a><a href="#account-devices">Devices & integrations</a><a href="#account-coverage">Data coverage</a></> : null}</nav><Link className="sidebar-help" href="/docs">Documentation <ArrowUpRight size={13} /></Link></aside><div className="account-card">
    <div className="account-card-head" id="overview"><div><span className="section-index">{account.workspace?.kind === "organization" ? "Organization workspace" : "Personal workspace"}{account.workspace?.role ? ` · ${account.workspace.role}` : ""}</span><h1>{account.workspace?.name || account.user.name || "Your UsageMax account"}</h1><p>{account.user.email || "Authenticated with WorkOS"}</p></div><button className="button button-outline" onClick={() => void signOut({ returnTo: "/" })} type="button">Sign out</button></div>
    ...
    {account.profile ? <>
      <section className="account-profile-row" id="account-profile">{editingProfile ? <form className="profile-edit-form" onSubmit={saveProfile}><label>Display name<input maxLength={80} onChange={(event) => setDisplayName(event.target.value)} required value={displayName} /></label><label>Bio<textarea maxLength={280} onChange={(event) => setBio(event.target.value)} rows={3} value={bio} /></label><div><button className="button button-dark" disabled={operation === "profile-edit"} type="submit">{operation === "profile-edit" ? "Saving…" : "Save profile"}</button><button className="button button-outline" onClick={() => setEditingProfile(false)} type="button">Cancel</button></div>{errors["profile-edit"] ? <p role="alert">{errors["profile-edit"]}</p> : null}</form> : <div><span>{account.profile.isPublic ? "Public profile" : "Private profile"}</span><strong>{account.profile.displayName} · @{account.profile.handle}</strong><small>{account.profile.bio || "No bio yet"}</small></div>}{!editingProfile && canManageProfile ? <div className="account-profile-actions"><button className="button button-outline" onClick={() => { setDisplayName(account.profile!.displayName); setBio(account.profile!.bio); setEditingProfile(true); }} type="button">Edit profile</button><button className="button button-outline" disabled={operation === "visibility"} onClick={() => void run("visibility", () => setVisibility({ isPublic: !account.profile!.isPublic }))} type="button">Make {account.profile.isPublic ? "private" : "public"}</button>{account.profile.isPublic ? <Link className="button button-dark" href={`/${account.profile.handle}`}>View public profile <ArrowUpRight size={15} /></Link> : null}</div> : null}</section>

      <section className="device-link-section" id="link-computer"><div className="device-link-copy"><h2>Link a computer</h2><p>Generate a one-use command, valid for ten minutes. Run it on the computer you want to sync.</p><div className="privacy-pills"><span>No prompts</span><span>No source code</span><span>No provider keys</span></div></div>{canManageOwnCollectors ? <form className="device-link-form" onSubmit={makeDeviceLink}><label htmlFor="device-name">Computer name</label><input id="device-name" maxLength={80} onChange={(event) => setDeviceName(event.target.value)} placeholder="Studio PC, MacBook, CI…" value={deviceName} /><button className="button button-primary" disabled={operation === "device-link"} type="submit">{operation === "device-link" ? "Generating…" : linkExpired ? "Generate new command" : "Generate link command"}</button><small>Works on macOS, Windows, Linux, and WSL.</small>{errors["device-link"] ? <p role="alert">{errors["device-link"]}</p> : null}</form> : <div className="device-link-form"><strong>Read-only access</strong><small>An organization admin can link and manage computers.</small></div>}</section>

      {canManageOwnCollectors && linkCode ? <div className={`link-command ${linkExpired ? "is-expired" : ""}`} role="status"><div><span>{linkStatus?.firstUploadAt ? "First sync confirmed" : linkStatus?.redeemedAt ? "Computer linked · waiting for first completed sync" : linkExpired ? "This command expired" : "Run this on the computer you want to link"}</span><code>{linkCommand}</code></div><button className="button button-dark" disabled={linkExpired || Boolean(linkStatus?.redeemedAt)} onClick={() => void copy(linkCommand, "link")} type="button">{copied === "link" ? "Copied" : "Copy command"}</button><small>Expires {absoluteTime(linkCode.expiresAt)} · {linkStatus?.syncPhase || "one use only"} · requires Bun or Node.js 20+</small></div> : null}
      {copyFallback ? <p className="copy-fallback" role="alert">Clipboard access failed. Select and copy: <span>{copyFallback}</span></p> : null}

      <details className="coverage-summary account-coverage" id="account-coverage" open={account.coverage?.phase === "failed"}><summary><span><i className={account.coverage?.status !== "complete" ? "is-partial" : ""} /><strong>Coverage and reconciliation</strong></span><span>{account.coverage?.completedAt ? `Completed ${relativeTime(account.coverage.completedAt)}` : "Coverage not assessed"}</span></summary><div>{account.coverage ? <dl><div><dt>Status</dt><dd>{account.coverage.status === "complete" ? "Parser-certified retained history" : account.coverage.status === "unverified" ? "Parser coverage not certified" : "Partial source inventory"}</dd></div><div><dt>Range</dt><dd>{account.coverage.from || "unknown"} → {account.coverage.to || "unknown"}</dd></div><div><dt>Sources</dt><dd>{account.coverage.sourceCount}</dd></div><div><dt>Partitions</dt><dd>{account.coverage.acceptedPartitions} / {account.coverage.partitionCount}</dd></div><div><dt>Corrections</dt><dd>{account.coverage.correctionRows} authoritative revision{account.coverage.correctionRows === 1 ? "" : "s"}</dd></div></dl> : <p>Run a full reconciliation before treating source count as complete coverage.</p>}<p>Check retained history: <code>bunx usagemax doctor --deep</code><br />Reconcile it: <code>bunx usagemax sync --full --explain</code></p></div></details>

      <section className="account-connections" id="account-devices"><div className="account-connections-head"><div><h2>Computers and integrations</h2></div><small>Sources appear after the first completed sync</small></div><div className="source-strip">{account.connectedSources.length ? account.connectedSources.map((source) => <span key={source}><i />{source}</span>) : <p>No local coding-agent sources reported yet.</p>}</div><div className="collector-list">{collectors.length ? collectors.map((collector) => <div className={collector.revokedAt ? "collector-row is-revoked" : "collector-row"} key={collector.id}><span className="collector-state"><i /><span>{renaming === collector.id ? <span className="collector-rename"><label className="sr-only" htmlFor={`rename-${collector.id}`}>Rename {collector.name}</label><input id={`rename-${collector.id}`} maxLength={80} onChange={(event) => setRenameValue(event.target.value)} value={renameValue} /><button onClick={() => void saveCollectorName(collector.id)} type="button">Save</button><button onClick={() => setRenaming(null)} type="button">Cancel</button></span> : <><strong>{collector.name}</strong><small>{collectorStatus(collector.lastSuccessAt, collector.revokedAt, collector.lastSyncPhase)} · {collector.keyPrefix}…</small></>}</span></span><span><small>{platformLabel(collector.platform)}{collector.cliVersion ? ` · CLI ${collector.cliVersion}` : ""}</small><strong>{absoluteTime(collector.lastSuccessAt)}</strong></span><span className="collector-actions">{collector.revokedAt ? <strong>Revoked</strong> : canManageOwnCollectors ? <><button aria-label={`Rename ${collector.name}`} disabled={Boolean(operation)} onClick={() => { setRenaming(collector.id); setRenameValue(collector.name); }} type="button">Rename</button>{collector.platform ? null : <button aria-label={`Rotate ${collector.name}`} disabled={Boolean(operation)} onClick={() => void rotate(collector.id)} type="button">Rotate</button>}<button aria-label={`Revoke ${collector.name}`} disabled={Boolean(operation)} onClick={() => void revoke(collector.id, collector.name)} type="button">Revoke</button></> : <strong>Read only</strong>}</span>{canManageOwnCollectors && (errors[`rename-${collector.id}`] || errors[`rotate-${collector.id}`] || errors[`revoke-${collector.id}`]) ? <p className="collector-error" role="alert">{errors[`rename-${collector.id}`] || errors[`rotate-${collector.id}`] || errors[`revoke-${collector.id}`]}</p> : null}{canManageOwnCollectors && issuedToken?.collectorId === collector.id ? <div className="collector-token" role="status"><span>New key ready to copy. It is shown once.</span><code>{issuedToken.token}</code><button className="button button-dark" onClick={() => void copy(issuedToken.token, `token-${collector.id}`)} type="button">{copied === `token-${collector.id}` ? "Copied" : "Copy key"}</button></div> : null}</div>) : <div className="collector-empty">No collectors yet. Generate a link command above.</div>}{collectorPage.status === "CanLoadMore" ? <button className="button button-outline collector-load-more" onClick={() => collectorPage.loadMore(25)} type="button">Load more collectors</button> : null}{collectorPage.status === "LoadingMore" ? <p className="collector-empty">Loading more collectors…</p> : null}</div></section>
      ...
      {canManageCollectors || canExport || canDelete ? <details className="account-data-controls"><summary>Data controls</summary><div>{canExport ? <button className="button button-outline" disabled={operation === "data-export"} onClick={() => void downloadExport()} type="button">{operation === "data-export" ? "Preparing export…" : "Download workspace data"}</button> : null}{canManageCollectors ? <div><p>Emergency access control: immediately stop every active computer, CI job, and integration from uploading to this workspace.</p><button className="button button-danger" disabled={Boolean(operation)} onClick={() => void revokeAll()} type="button">{operation === "revoke-all" ? "Revoking all collectors…" : "Revoke all collector access"}</button></div> : null}{canDelete ? account.deletionRequest ? <div>
        {account.deletionRequest.waitingForBillingCancellation ? <div role="status"><strong>Deletion is paused until billing is resolved.</strong><p>An active or unresolved subscription remains on this workspace. Your account and local billing reference are still in place. UsageMax will not cancel the subscription for you. Manage it in the billing portal and cancel it if needed; deletion resumes after Stripe confirms a canceled or inactive status.</p>{account.deletionRequest.billingPortalAvailable ? <form action="/api/billing/portal" method="post"><button className="button button-primary" type="submit">Manage billing</button></form> : <p>Billing portal access is unavailable. Contact <a href="mailto:hello@usagemax.com">UsageMax billing support</a> before continuing.</p>}</div> : account.deletionRequest.billingCancellationConfirmed ? <p role="status">Billing is canceled or inactive. Account deletion is still in progress and will resume automatically within 24 hours.</p> : <p>Account deletion is scheduled for {absoluteTime(account.deletionRequest.scheduledFor)}. Collector uploads are disabled during the seven-day recovery window.</p>}
        <button className="button button-outline" onClick={() => void run("delete-cancel", () => cancelDeletion({}))} type="button">Cancel deletion and restore collectors</button>
      </div> : <form onSubmit={(event) => { event.preventDefault(); void run("delete-request", () => requestDeletion({ confirmation: deleteConfirmation })); }}><label>Schedule account deletion in seven days<input onChange={(event) => setDeleteConfirmation(event.target.value)} placeholder="delete my account" value={deleteConfirmation} /></label><button className="button button-danger" disabled={operation === "delete-request"} type="submit">Schedule deletion</button></form> : null}{errors["data-export"] ? <p role="alert">{errors["data-export"]}</p> : null}{errors["revoke-all"] ? <p role="alert">{errors["revoke-all"]}</p> : null}{errors["delete-request"] ? <p role="alert">{errors["delete-request"]}</p> : null}</div></details> : null}
...
export function AccountView() {
  return <section className="account-page shell"><AuthLoading><div className="account-loading" role="status">Loading your secure workspace…</div></AuthLoading><Authenticated><AccountPanel /></Authenticated><Unauthenticated><div className="account-signin"><h1>Sign in</h1><p>Use GitHub or Google.</p><AuthNavigation className="button button-dark" href="/sign-in">Continue to secure sign in <ArrowUpRight size={15} /></AuthNavigation></div></Unauthenticated></section>;
}
