...
function AccountPanel() {
  ...
  const exportData = useQuery(api.account.exportAccount, exportRequested ? {} : "skip");
  ...
  const computers = account.collectors.filter((collector) => collector.platform && !collector.revokedAt);
  const firstSyncConfirmed = account.collectors.some((collector) => Boolean(collector.lastSuccessAt));
  const handleSuggestion = suggestHandle(account.user.name, account.user.email) || "your-handle";

  return <div className="account-card">
    <div className="account-card-head"><div><span className="section-index">Personal workspace</span><h1>{account.user.name || "Your UsageMax account"}</h1><p>{account.user.email || "Authenticated with WorkOS"}</p></div><button className="button button-outline" onClick={() => void signOut({ returnTo: "/" })} type="button">Sign out</button></div>
    ...
    {account.profile ? <>
      ...
      <section className="account-connections"><div className="account-connections-head"><div><span className="section-index">Connected activity</span><h2>Computers and integrations.</h2></div><small>Sources appear after the first completed sync</small></div><div className="source-strip">{account.connectedSources.length ? account.connectedSources.map((source) => <span key={source}><i />{source}</span>) : <p>No local coding-agent sources reported yet.</p>}</div><div className="collector-list">{account.collectors.length ? account.collectors.map((collector) => <div className={collector.revokedAt ? "collector-row is-revoked" : "collector-row"} key={collector.id}><span className="collector-state"><i /><span>{renaming === collector.id ? <span className="collector-rename"><label className="sr-only" htmlFor={`rename-${collector.id}`}>Rename {collector.name}</label><input id={`rename-${collector.id}`} maxLength={80} onChange={(event) => setRenameValue(event.target.value)} value={renameValue} /><button onClick={() => void saveCollectorName(collector.id)} type="button">Save</button><button onClick={() => setRenaming(null)} type="button">Cancel</button></span> : <><strong>{collector.name}</strong><small>{collectorStatus(collector.lastSuccessAt, collector.revokedAt, collector.lastSyncPhase)} · {collector.keyPrefix}…</small></>}</span></span><span><small>{platformLabel(collector.platform)}{collector.cliVersion ? ` · CLI ${collector.cliVersion}` : ""}</small><strong>{absoluteTime(collector.lastSuccessAt)}</strong></span><span className="collector-actions">{collector.revokedAt ? <strong>Revoked</strong> : <><button aria-label={`Rename ${collector.name}`} disabled={Boolean(operation)} onClick={() => { setRenaming(collector.id); setRenameValue(collector.name); }} type="button">Rename</button>{collector.platform ? null : <button aria-label={`Rotate ${collector.name}`} disabled={Boolean(operation)} onClick={() => void rotate(collector.id)} type="button">Rotate</button>}<button aria-label={`Revoke ${collector.name}`} disabled={Boolean(operation)} onClick={() => void revoke(collector.id, collector.name)} type="button">Revoke</button></>}</span>{errors[`rename-${collector.id}`] || errors[`rotate-${collector.id}`] || errors[`revoke-${collector.id}`] ? <p className="collector-error" role="alert">{errors[`rename-${collector.id}`] || errors[`rotate-${collector.id}`] || errors[`revoke-${collector.id}`]}</p> : null}{issuedToken?.collectorId === collector.id ? <div className="collector-token" role="status"><span>New key ready to copy. It is shown once.</span><code>{issuedToken.token}</code><button className="button button-dark" onClick={() => void copy(issuedToken.token, `token-${collector.id}`)} type="button">{copied === `token-${collector.id}` ? "Copied" : "Copy key"}</button></div> : null}</div>) : <div className="collector-empty">No collectors yet. Generate a link command above.</div>}</div></section>

      <details className="advanced-collector"><summary>Advanced · custom telemetry collector</summary><div><p>Create a raw collector key for an SDK, CI job, or OpenTelemetry pipeline. The secret is shown once.</p><form onSubmit={addCollector}><label htmlFor="integration-name">Integration name</label><input id="integration-name" maxLength={80} onChange={(event) => setCollectorName(event.target.value)} placeholder="Production OTEL, CI, custom agent…" value={collectorName} /><button className="button button-outline" disabled={operation === "collector-create"} type="submit">{operation === "collector-create" ? "Creating…" : "Create API key"}</button></form></div>{issuedToken && !issuedToken.collectorId ? <div className="collector-token" role="status"><span>Key ready to copy. UsageMax stores only its SHA-256 hash.</span><code>{issuedToken.token}</code><button className="button button-dark" onClick={() => void copy(issuedToken.token, "token-new")} type="button">{copied === "token-new" ? "Copied" : "Copy key"}</button></div> : null}</details>

      <details className="account-data-controls"><summary>Data controls</summary><div><button className="button button-outline" disabled={exportRequested} onClick={() => setExportRequested(true)} type="button">{exportRequested ? "Preparing export…" : "Download account data"}</button>{account.deletionRequest ? <div><p>Hard deletion is scheduled for {absoluteTime(account.deletionRequest.scheduledFor)}. Collector uploads are disabled during the seven-day recovery window.</p><button className="button button-outline" onClick={() => void run("delete-cancel", () => cancelDeletion({}))} type="button">Cancel deletion and restore collectors</button></div> : <form onSubmit={(event) => { event.preventDefault(); void run("delete-request", () => requestDeletion({ confirmation: deleteConfirmation })); }}><label>Schedule account deletion in seven days<input onChange={(event) => setDeleteConfirmation(event.target.value)} placeholder="delete my account" value={deleteConfirmation} /></label><button className="button button-danger" disabled={operation === "delete-request"} type="submit">Schedule deletion</button></form>}{errors["delete-request"] ? <p role="alert">{errors["delete-request"]}</p> : null}</div></details>
