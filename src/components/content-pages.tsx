    ...
    <div className="page-surface content-page docs-page">
      ...
      <div className="shell docs-layout">
        ...
        <article className="docs-article">
          <section className="docs-section" id="quickstart"><div className="docs-section-kicker">01 / QUICKSTART</div><h2>Ship one event in under a minute.</h2><p>UsageMax accepts a bounded batch of model, tool, state, and outcome events. Give each event a stable key and the smallest useful amount of context.</p><CodeBlock label="terminal / native telemetry">{quickstart}</CodeBlock><Callout tone="cyan">Use a dedicated collector key. Never put provider credentials or prompt content in the public event payload.</Callout></section>
          <section className="docs-section" id="event-contract"><div className="docs-section-kicker">02 / EVENT CONTRACT</div><h2>Small fields. Strong edges.</h2><p>Three required fields identify what happened; optional fields make the trace easier to follow. Token and cost fields are numeric so the public rollups stay deterministic.</p><div className="contract-table"><div><code>eventKey</code><span>stable, idempotent event identifier</span><b>required</b></div><div><code>model</code><span>the model actually used for the event</span><b>required</b></div><div><code>occurredAt</code><span>ISO timestamp or Unix milliseconds</span><b>required</b></div><div><code>eventType</code><span>model_request / tool_call / agent_state / outcome</span><b>optional</b></div><div><code>source</code><span>the runtime or product emitting the event</span><b>optional</b></div><div><code>status</code><span>ok / error / cancelled</span><b>optional</b></div><div><code>traceId</code><span>optional link back to your private trace system</span><b>optional</b></div></div></section>
...
export function MethodologyView() {
  return (
    <div className="page-surface content-page methodology-page">
      ...
      <section className="shell content-section methodology-table-section"><div className="section-heading"><div className="eyebrow"><span className="eyebrow-line" />Metric notes</div><h2>What each number <span className="text-accent">means.</span></h2></div><div className="methodology-table"><div className="methodology-table-head"><span>Metric</span><span>Definition</span><span>Surface</span></div><div><strong>Total tokens</strong><span>The provider-reported total, or input plus output when no total is supplied. Cache and reasoning remain visible dimensions without being double-counted.</span><code>profile / daily</code></div><div><strong>Indexed spend</strong><span>Provider cost in micros, displayed as USD after aggregation.</span><code>network / profile</code></div><div><strong>Active agent</strong><span>A live agent row whose heartbeat has not passed its expiry window.</span><code>network / live</code></div><div><strong>Signal score</strong><span>The selected leaderboard metric for the chosen period, ordered descending.</span><code>leaderboard</code></div></div></section>
