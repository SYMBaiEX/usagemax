...
function HeroVisual({ network, pulse }: { network: NetworkData | null | undefined; pulse: PulseRow[] | undefined }) {
  const peak = Math.max(...(pulse?.map((row) => row.totalTokens) ?? []), 1);
  return <div className="usage-preview" aria-label="UsageMax public network summary">
    ...
    <div className="usage-preview-chart" aria-label="Scale of recently updated public profiles">{pulse?.map((row) => <i key={row.handle} title={`@${row.handle} · ${compactNumber(row.totalTokens, 2)} tokens`} style={{ "--bar-height": `${Math.max(3, (row.totalTokens / peak) * 100)}%` } as CSSProperties} />)}</div>
