...
export const USAGEMAX_TOOLS = [
  { name: "public_profile", title: "Read public profile", description: "Read one opt-in public UsageMax profile and aggregate statistics.", inputSchema: { type: "object", properties: { handle: { type: "string", minLength: 1, maxLength: 80, description: "The public profile handle, without the @ prefix." } }, required: ["handle"], additionalProperties: false }, outputSchema: jsonObjectOutput, annotations: readOnlyAnnotations },
  { name: "leaderboard", title: "Read public leaderboard", description: "Read the public UsageMax leaderboard (maximum 100 rows).", inputSchema: { type: "object", properties: { metric: { type: "string", enum: ["tokens", "spend"], default: "tokens", description: "Rank by aggregate tokens or tracked spend." }, period: { type: "string", enum: ["7d", "30d", "all"], default: "all", description: "Choose the bounded reporting window." } }, required: [], additionalProperties: false }, outputSchema: jsonObjectOutput, annotations: readOnlyAnnotations },
  { name: "network_stats", title: "Read network statistics", description: "Read aggregate public UsageMax network statistics and render the optional inline observability view.", inputSchema: { type: "object", properties: {}, required: [], additionalProperties: false }, outputSchema: jsonObjectOutput, annotations: readOnlyAnnotations, _meta: { ui: { resourceUri: MCP_APP_RESOURCE_URI } } },
...
export const DOC_TOOL_DEFINITIONS = [
  ...
  { name: "docs_get", title: "Read UsageMax documentation", description: "Retrieve one bounded public UsageMax documentation resource.", inputSchema: { type: "object", properties: { id: { type: "string", enum: Object.keys(DOCS), description: "The documentation resource identifier returned by docs_list." } }, required: ["id"], additionalProperties: false }, outputSchema: jsonObjectOutput, annotations: readOnlyAnnotations },
];
