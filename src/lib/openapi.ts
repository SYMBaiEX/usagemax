...
const openapi = {
  ...
  paths: {
    ...
    "/api/v2/usage/snapshots": { post: { tags: ["CLI ingestion"], operationId: "uploadUsageSnapshot", description: "Process one authenticated snapshot operation: begin, sessions, partitions, complete, or fail. Requires JSON, bearer token, matching device UUID, and a 2 MiB maximum request. Sessions are capped at 100, partitions at 10 per request, and rows at 100 per partition; idempotent replay and correction conflicts are explicit.", security: [{ collectorBearer: [] }], parameters: [deviceHeader], requestBody: { ...json, content: { "application/json": { schema: { $ref: "#/components/schemas/SnapshotOperation" }, example: { operation: "begin", runId: "run_2026_09_15_01", mode: "incremental", sourceCount: 1, partitionCount: 1, inventoryComplete: true } } } }, responses: { "200": response("Completed, failed, or replayed operation.", { $ref: "#/components/schemas/SnapshotResponse" }), "202": response("Accepted snapshot operation; the response is the receipt for the current begin, sessions, or partition write. The client continues the same run and sends complete to publish it; the response includes a read-only statusUrl and Location header for the authenticated run-status resource.", { $ref: "#/components/schemas/SnapshotResponse" }), "400": errorResponse("invalid_snapshot or request"), "401": errorResponse("unauthorized"), "409": errorResponse("snapshot_conflict or device_identity_mismatch"), "410": errorResponse("snapshot_run_expired"), "413": errorResponse("payload_too_large"), "415": errorResponse("content_type_must_be_application_json"), "429": errorResponse("rate_limited") } } }
  },
